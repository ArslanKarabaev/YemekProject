...
from canteen.models import Yemek
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        gallery_rows = soup.find_all('div', class_='row mt-4')
        ...
        for gallery_row in gallery_rows:
            ...
            meal_cards = gallery_row.find_all('div', class_='item features-image сol-12 col-md-6 col-lg-3')

            for card in meal_cards:
                try:
                    # Название блюда
                    name = card.find('h5', class_='item-title mbr-fonts-style display-7').text.strip()
                    ...
                    count += 1
                    self.stdout.write(self.style.SUCCESS(f"Добавлено: {name} - {image_url}"))
